...
um-vpn connects to the University of Macau SSL VPN. It fills in the UMPASS
sign-in from the login keyring, reads the portal's `DSID` session cookie from
Chrome over DevTools, and hands it to NetworkManager's openconnect plugin. All
of it is in `um_vpn.py`. README.md is for users; docs/decisions.md records why
it is built this way and what failed before, and docs/debugging.md shows how to
find out why a tunnel failed.
...
- Don't run `um-vpn on|off|remember|forget` or
  `nmcli connection up|down|delete`. Each costs the user a real Duo push, or
  changes their real network or their stored sign-in. `um-vpn status` and bare
  `um-vpn`, which only shows help, are safe; ask the user to run the rest.
